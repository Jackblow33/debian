...
if lspci | grep -i "nvidia" > /dev/null; then
    echo "NVIDIA GPU detected. Installing NVIDIA driver..."
    ...
    apt install pkg-config libglvnd-dev dkms build-essential libegl-dev libegl1 libgl-dev libgl1 libgles-dev libgles1 libglvnd-core-dev libglx-dev
    apt install libopengl-dev gcc make pkg-config linux-headers-$(uname -r)
